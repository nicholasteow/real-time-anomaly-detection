...
Date          : 26 July 2024
Purpose       : Script collects and exposes power meter metrics from a MongoDB 
...
power_metric = Gauge(f'mongodb_{collection_name_sanitized}_power', 'Power value from MongoDB')
...
def collect_metrics():
    try:
        ...
        cursor = collection.find({}, {"power": 1}).sort("date_time", pymongo.ASCENDING)
        
        for doc in cursor:
            power_value = doc.get('power', 0)
            
            # Update Prometheus metric
            power_metric.set(power_value)
